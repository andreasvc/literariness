...
def inducedfeatures():
	...
	# re-index because texts with less than 1000 sentences need to filtered
	target = pandas.read_csv('features/target.csv', index_col=0
			).ix[simple.index]

	result = pandas.DataFrame(index=simple.index)
	...
	if os.path.exists('features/fragcounts.npz'):
		...
		fragcounts = fragcounts[index.get_indexer(simple.index), :]
		print(fragcounts.shape)
		...
		for n in sorted(target.fold.unique()):
			...
			scaler = feature_extraction.text.TfidfTransformer(
					norm='l2', use_idf=USE_IDF, smooth_idf=SMOOTH_IDF,
					sublinear_tf=SUBLINEAR_TF)
			# fit scaler only on training data, apply same transform to test data
			X_train = scaler.fit_transform(X_train)
			X_test = scaler.transform(X_test)
			y_train = target[outsidefold].target
			if discrete:
				y_train = le.transform(y_train)
			model = (svm.LinearSVC(C=C, random_state=1)
				if discrete else
				svm.LinearSVR(C=C, epsilon=epsilon,
					loss='epsilon_insensitive', random_state=1))
			model.fit(X_train, y_train)
		...
		for fold in sorted(target.fold.unique()):
			...
			for a in seq:
				...
				if x == 'topics':  # add columns for all topics
					tmp1 = data[data.columns[data.columns.str.match(
							r't[0-9]{1,2}', as_indexer=True)]]
					assert tmp1.shape[1] > 0
					tmp[tmp1.columns] = tmp1
			...
			X = dv.fit_transform([a for _, a in tmp.iterrows()])
			infold = target.fold == fold
			outsidefold = target.fold != fold
			if discrete:
				# fit scaler only on training data,
				# apply same transform to test data
				X_train = scaler.fit_transform(X[outsidefold.nonzero()[0], :])
				X_test = scaler.transform(X[infold.nonzero()[0], :])
			else:
				X_train = X[outsidefold.nonzero()[0], :]
				X_test = X[infold.nonzero()[0], :]
			y_train = target[outsidefold].target
			if discrete:
				y_train = le.transform(y_train)
				model = linear_model.LogisticRegressionCV(random_state=1)
			else:
				model = linear_model.RidgeCV(normalize=True)
			model.fit(X_train, y_train)
			foldpred = model.predict(X_test)
			if discrete:
				foldpred = le.inverse_transform(foldpred)
